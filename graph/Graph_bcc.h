        ...
        switch ( status ( u ) ) { //并视u的状态分别处理
            case UNDISCOVERED:
                ...
                if ( hca ( u ) < dTime ( v ) ) //遍历返回后，若发现u（通过后向边）可指向v的真祖先
                    ...
                    /*DSA*/stack<int> temp; do { temp.push (S.top() );S.pop(); print ( vertex ( temp.top() ) ); } while ( u != temp.top() ); print( vertex ( parent(u) ) ); while ( !temp.empty() ) S.push ( temp.pop() );
                    while ( u != S.top() )S.pop(); //弹出当前BCC中（除v外）的所有节点，可视需要做进一步处理
                    /*DSA*/printf ( "\n" );
                    /*DSA*/}
                break;
            case DISCOVERED:
